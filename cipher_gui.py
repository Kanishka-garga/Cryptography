...
# # Load saved models and other components using joblib where applicable
...
# # Load neural network and LSTM models saved with Keras
# neural_network_model = models.load_model("neural_network_model.h5")
# lstm_model = models.load_model("lstm_model.h5")

# # Load BERT model and tokenizer (saved with PyTorch/Transformers library)
...
#     try:
#         # Vectorize the input ciphertext for Naive Bayes, Neural Network, and LSTM
...
#         # Store all predictions and confidences for GUI
#         results = {
...
#         # Prepare result message for all models
#         result_message = (
...
#     # Buttons to show predictions from different models with confidence scores
...
def predict_cipher(ciphertext):
    try:
        ...
        transformer_probs = torch.softmax(outputs.logits, dim=1).numpy()[0]
        transformer_prediction = label_encoder.inverse_transform([transformer_probs.argmax()])[0]  # Make sure the correct label is used
        transformer_confidence = transformer_probs.max()
